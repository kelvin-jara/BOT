// THIS VERSION INLCUDES THE FOLLOWING CHANGES:
// 1. THIS VERSION OPENS A CONTRARY POSITION WHEN THE LVL NUMBER OF GRID LEVELS IS N = 2, 3, ETC.
...
public:
    ...
    void CheckForGrid() {
        checkForGrid( this, magicNumber, risk, gridVolMult, gridMaxLvl, slippage, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling);
    }

    void CheckForReverseGrid() {
        checkForReverseGrid( this, magicNumber, risk, gridVolMult, gridMaxLvl, slippage, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling);
    }
...
//|                                                                  |
//+------------------------------------------------------------------+
void checkForGrid(GerEA &parentEA, ulong magic, double risk, double volCoef, int maxLvl, int slippage = 30, int nRetry = 5, int mRetry = 2000, bool news = false, ENUM_NEWS_IMPORTANCE newsImportance = NEWS_IMPORTANCE_MEDIUM, int newsMinsBefore = 60, int newsMinsAfter = 60, ENUM_FILLING filling = FILLING_DEFAULT) {
    int minPoints = 30;
    MqlTradeRequest req;
    MqlTradeResult res;
    int total = PositionsTotal();
    ...
    for (int i = total - 1; i >= 0; i--) {
        ...
        if (ptype == POSITION_TYPE_BUY) {
            ...
            double ssl, stp;
            ssl = NormalizeDouble(Bid(psymbol) + pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            stp = NormalizeDouble(Bid(psymbol) - pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            string commentSell = DoubleToString(pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            if(n == 2 ){
                bool orderOk;
                orderOk = order(ORDER_TYPE_SELL, anotherMagic, Bid(psymbol), 0, 0, risk, false, 0, slippage, false, false, commentSell, psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling);
                Print("BUY reversing grid:", orderOk);
                parentEA.reverseGrid = true;
            }else if(n > 2){
                bool orderOk;
                orderOk = order(ORDER_TYPE_SELL, anotherMagic, Bid(psymbol), 0, 0, risk, false, 0, slippage, false, false, "", psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling);
                Print("BUY reversing grid:", orderOk);
                parentEA.reverseGrid = true;
            }
            ...
            double ssl, stp;
            ssl = NormalizeDouble(Ask(psymbol) - pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            stp = NormalizeDouble(Ask(psymbol) + pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            string commentSell = DoubleToString(pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            if(n ==2 ){
                bool orderOK;
                orderOK = order(ORDER_TYPE_BUY, anotherMagic, Ask(psymbol), 0, 0, risk, false, 0, slippage, false, false, commentSell, psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling);
                Print("SELL reversing grid: ", orderOK);
                parentEA.reverseGrid = true;
            }  else if(n > 2){
                bool orderOK;
                orderOK = order(ORDER_TYPE_BUY, anotherMagic, Ask(psymbol), 0, 0, risk, false, 0, slippage, false, false, "", psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling);
                Print("SELL reversing grid: ", orderOK);
                parentEA.reverseGrid = true;
            }
        ...
        for (int j = 0; j < n; j++) {
            ...
            ZeroMemory(res);
            req.position = tickets[j];
            ulong magicNumberk = PositionGetInteger(POSITION_MAGIC);
            Print("Order Magic Number: ", magicNumberk);
...
//|                                                                  |
//+------------------------------------------------------------------+
void checkForReverseGrid(GerEA &parentEA, ulong magic, double risk, double volCoef, int maxLvl, int slippage = 30, int nRetry = 5, int mRetry = 2000, bool news = false, ENUM_NEWS_IMPORTANCE newsImportance = NEWS_IMPORTANCE_MEDIUM, int newsMinsBefore = 60, int newsMinsAfter = 60, ENUM_FILLING filling = FILLING_DEFAULT) {
    int minPoints = 30;
    MqlTradeRequest req;
    MqlTradeResult res;
    ...
    for (int i = total - 1; i >= 0; i--) {
        ...
        if (ptype == POSITION_TYPE_BUY) {
            ...
            stp = NormalizeDouble(Bid(psymbol) - pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            string commentSell = DoubleToString(pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            if(n == 2 && order(ORDER_TYPE_SELL, pmagic, Bid(psymbol), 0, 0, risk, false, 0, slippage, false, false, commentSell, psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling)){
                Print("revering grid");
                parentEA.reverseGrid = true;
            } 
            ...
            stp = NormalizeDouble(Ask(psymbol) + pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            string commentSell = DoubleToString(pd, (int)SymbolInfoInteger(psymbol, SYMBOL_DIGITS));
            if(n == 2 && order(ORDER_TYPE_BUY, pmagic, Ask(psymbol), 0, 0, risk, false, 0, slippage, false, false, commentSell, psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling)){
                Print("revering grid");
                parentEA.reverseGrid = true;
            }
