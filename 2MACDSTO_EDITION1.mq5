...
//this version has revrese grid included
#include <EAUtils_EDITED3.mqh>
...
//+------------------------------------------------------------------+
int OnInit() {
    Print("Current Terminal Path: ", TerminalInfoString(TERMINAL_DATA_PATH));
...
void OnTimer() {
    ...
    tc = TimeCurrent();
    if (tc == oldTc) return;
    if (Trail && !ea.reverseGrid) ea.CheckForTrail();
    if (EquityDrawdownLimit) ea.CheckForEquity();
    if (Grid) ea.CheckForGrid();  
    if(ea.reverseGrid) ea.CheckForReverseGrid();
